...
async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    const events: LearningEvent[] = Array.isArray(input.events)
      ? input.events
      : input.type && input.context && input.detail
        ? [input as LearningEvent]
        : [];

    if (events.length === 0) {
      return errorResponse(res, 'Invalid input: provide either a single event {type, context, detail} or {events: [...]}', 400);
    }
    ...
    for (let i = 0; i < events.length; i++) {
      ...
      if (!e.type || !VALID_TYPES.includes(e.type)) {
        return errorResponse(res, `Event ${i}: invalid type "${e.type}". Must be one of: ${VALID_TYPES.join(', ')}`, 400);
      }
      if (!e.context || typeof e.context !== 'string') {
        return errorResponse(res, `Event ${i}: "context" is required and must be a string`, 400);
      }
      if (!e.detail || typeof e.detail !== 'string') {
        return errorResponse(res, `Event ${i}: "detail" is required and must be a string`, 400);
      }
    }
    ...
    return successResponse(res, {
      ...
      _meta: {
        ...
        events_processed: entries.length,
      }
    });
  } catch (error: any) {
    console.error('[self-improving-agent] Processing error:', error.message);
