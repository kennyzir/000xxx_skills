...
/**
 * capability-evolver — Pure Logic Skill (Self-Hosted)
 ...
 *
 * 类型: 纯逻辑处理 (无外部 API 依赖，完全自营)
 * 标签: self-improvement, evolution, agent-meta, self-repair, analysis
 * License: MIT
 *
 * Actions:
...
async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const input = validation.data!.input as EvolveRequest;

  const reqValidation = validateRequest(input);
  if (!reqValidation.valid) {
    return errorResponse(res, 'Invalid request', 400, reqValidation.errors);
  }
  ...
  try {
    ...
    switch (input.action) {
      case 'analyze':
        result = handleAnalyze(input.logs!);
        break;
      case 'evolve':
        result = handleEvolve(input.logs!, input.strategy || 'auto', input.target_file);
        break;
      case 'status':
        result = handleStatus();
        break;
    ...
    console.error('[capability-evolver] Error:', error.message);
    return errorResponse(res, error.message || 'Processing failed', 500);
  }
}
