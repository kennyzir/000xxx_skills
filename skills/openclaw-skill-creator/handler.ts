...
/**
 * openclaw-skill-creator — Create Custom Skills for OpenClaw Agents
 * Pure logic skill that helps non-technical users create skills for their personal OpenClaw agents.
 * Focus: User-friendly, scenario-driven, plain English guidance.
 *
 * Actions: create, get-api-key-help, test-skill, improve
 */

const VALID_ACTIONS = ['create', 'get-api-key-help', 'test-skill', 'improve'] as const;
type Action = typeof VALID_ACTIONS[number];

// ─── Skill Templates for Common Scenarios ───
...
function createSkill(input: any): any {
  ...
  const whatYouHave = input.what_you_have || '';

  if (!whatYouWant) {
    throw { status: 400, message: 'Please tell me what you want your agent to do (what_you_want field)' };
  }
...
function getApiKeyHelp(input: any): any {
...
function testSkill(input: any): any {
  const skillName = input.skill_name || '';
  const testPrompt = input.test_prompt || '';

  if (!skillName) {
    throw { status: 400, message: 'Please specify which skill to test (skill_name field)' };
  }
...
function improveSkill(input: any): any {
  ...
  if (!skillName) {
    throw { status: 400, message: 'Please specify which skill to improve (skill_name field)' };
  }

  if (!feedback && !whatToImprove) {
    throw { status: 400, message: 'Please tell me what you want to improve (feedback or what_to_improve field)' };
  }
  ...
  try {
    ...
    switch (action) {
      ...
      case 'get-api-key-help':
        result = getApiKeyHelp(input);
        break;
      case 'test-skill':
        result = testSkill(input);
        break;
      case 'improve':
    ...
    return successResponse(res, {
      ...result,
      _meta: { skill: 'openclaw-skill-creator', latency_ms: Date.now() - startTime, version: '1.0.0' },
    });
  } catch (error: any) {
    if (error.status) {
      return errorResponse(res, error.message, error.status);
    }
    console.error('[openclaw-skill-creator] Error:', error.message);
